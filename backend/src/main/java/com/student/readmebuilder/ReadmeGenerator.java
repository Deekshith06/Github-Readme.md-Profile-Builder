...
public class ReadmeGenerator {
    ...
    private String getCategory(String skill) {
        ...
        for (String k : new String[] { "matplotlib", "seaborn", "plotly", "tableau", "power bi", "powerbi", "d3.js",
                "d3 ", "bokeh", "altair", "grafana", "kibana", "looker", "metabase", "superset", "excel" })
            if (s.contains(k.trim()))
                return "DATAVIZ";

        for (String k : new String[] { "tensorflow", "pytorch", "keras", "scikit", "sklearn", "pandas", "numpy",
                "opencv", "huggingface", "nltk", "spacy", "xgboost", "lightgbm", "catboost", "onnx", "mlflow", "scipy",
                "statsmodels", "transformers", "langchain", "dask" })
            if (s.contains(k.trim()))
                return "AI";

        for (String k : new String[] { "react", "vue", "angular", "next", "nuxt", "svelte", "gatsby", "astro", "node",
                "express", "django", "flask", "fastapi", "spring", "laravel", "rails", "nestjs", "html", "css",
                "bootstrap", "tailwind", "jquery", "graphql", "webpack", "vite", "redux", "deno", "bun" })
            if (s.contains(k.trim()))
                return "WEB";

        for (String k : new String[] { "mysql", "postgresql", "postgres", "mongodb", "redis", "sqlite", "oracle",
                "cassandra", "elasticsearch", "dynamodb", "neo4j", "influxdb", "mariadb", "snowflake", "bigquery",
                "prisma", "hadoop", "spark", "hive" })
            if (s.contains(k.trim()))
                return "DB";

        for (String k : new String[] { "aws", "azure", "gcp", "google cloud", "docker", "kubernetes", "jenkins",
                "terraform", "ansible", "circleci", "heroku", "vercel", "netlify", "firebase", "supabase", "nginx",
                "github actions", "cloudflare", "digitalocean", "travis", "devops", "ci/cd", "apache", "ubuntu" })
            if (s.contains(k.trim()))
                return "CLOUD";

        for (String k : new String[] { "machine learning", "deep learning", "computer vision", "nlp",
                "natural language", "data structure", "algorithm", "oop", "microservice", "system design", "agile",
                "scrum", "mlops", "blockchain", "iot", "embedded", "robotics", "cybersecurity", "neural network",
                "reinforcement" })
            if (s.contains(k.trim()))
                return "CONCEPTS";

        for (String k : new String[] { "git", "gitlab", "bitbucket", "vscode", "vs code", "intellij", "pycharm",
                "jupyter", "postman", "figma", "arduino", "unity", "unreal", "blender", "photoshop", "illustrator",
                "jira", "notion", "slack", "android", "raspberry", "windows", "macos", "linux" })
            if (s.contains(k.trim()))
                return "TOOLS";

        // Languages last
        for (String k : new String[] { "python", "java", "javascript", "typescript", "c++", "c#", "golang", "rust",
                "kotlin", "swift", "php", "ruby", "dart", "scala", "perl", "haskell", "lua", "matlab", "bash", "shell",
                "groovy", "elixir", "fortran", "clojure", "erlang", "julia", "nim", "zig", "solidity", "cobol", "vba" })
            if (s.contains(k.trim()))
                return "LANG";
    ...
    public String generate(String name, String title, String about,
        ...
        String encAbout = about.replace(" ", "%20").substring(0, Math.min(about.length(), 50));
        sb.append(
                ...
                .append(encTitle).append("&fontSize=45&fontColor=fff&animation=twinkling&fontAlignY=35&desc=")
